				...
				$.map(this, function (el, idx) {
					...
					var treeRoot = function (className) {
						...
						if (className) {
							...
							return x.obj.find('ul.' + nm);
						}
					}

					x.obj = $(el);
					x.name = x.name || function (obj) {
						return obj.name;
					...
					x.cbcs = {
							...
							try {
								...
								var result2 = function () {
									try {
										...
										try {
											(x.callbacks && x.callbacks.click) && x.callbacks.click(elem);
										} catch (e) {
											alert(e);
										}
						...
						mouseover : function (evt) {
							...
							try {
								(x.callbacks && x.callbacks.mouseover) && x.callbacks.mouseover(elem);
							} catch (e) {
								alert(e);
							}
						},
						mouseout : function (evt) {
							...
							try {
								(x.callbacks && x.callbacks.mouseout) && x.callbacks.mouseout(elem);
							} catch (e) {
								alert(e);
							}
						}
					...
					var getPath = function (leaf, parent) {
						if (leaf) {
							...
							return {
								...
								strp : strp,
								arr : arr,
								leaf : parent ? (leaf.parent ? leaf.parent : treeViewModel) : leaf
							};
						} else {
							return {
								str : '/' + treeViewModel.name,
								strp : '/',
								arr : [treeViewModel],
								leaf : treeViewModel
							};
								...
								if (leaf.children.length == 0) {
									(function (leaf, cb) {
										window.setTimeout(function () {
											controlObject.leaf(leaf, cb);
										}, x.animate);
									})(leaf, addCallback);
								} else {
									leaf.elem.ul.slideDown(x.animate);
								}
							} else {
								gsCook(leaf); // DELETE
					...
					var parseLeaf = function (leaf, obj, pl_callback) { // where to add, what JSON to add,
						try {
							...
							for (var i = 0; i < obj.length; i++) {
								leaf.children.push(makeLeaf(obj[i], leaf));
							}
							...
							object.slideDown(x.animate, function () {
								...
								if (x.preloader > 0) {
									x.obj.removeClass(x.classes.preloader);
								}
							});
						} catch (e) {
							alert(e);
					...
					var controlObject = {
						leaf : function (leaf, callback) {
							try {
								...
								if (x.preloader > 1) {
									x.obj.addClass(x.classes.preloader);
								}
						...
						shLoader : function (leaf, tf) {
							try {
								if (leaf.elem.heading) {
									if (tf) {
										leaf.elem.heading.addClass(x.classes.loader);
									} else {
										leaf.elem.heading.removeClass(x.classes.loader);
									}
								}
							} catch (e) {
								alert(e);
